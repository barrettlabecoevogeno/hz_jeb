...
thurman.geno <- read.csv("processed_data/joint_transect.csv") %>% 
  filter(year == "2015") %>%
...
none <- stan("src/stan_models/multinomial/multi_free_none.stan",
             data = list(N = dim(thurman.geno)[1], 
                         genos = as.matrix(thurman.geno[,13:15]),
                         transectDist = thurman.geno$transect.dist),
...
save(right, file = "results/thurman_bestCline.Rdata")

# and save all model results for other uses
save(none, left, right, mirror, ind, file = "results/thurman_allClines.Rdata")
